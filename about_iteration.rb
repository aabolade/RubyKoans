...
class AboutIteration < Neo::Koan
  ...
  def test_inject_will_blow_your_mind
    ...
    # given an array [a,b,c].inject(initial) {|inital, item| initial (operation) item }

    # e.g [3, 4, 5].inject(10) {|product, item| product * item } = 10 * 3 * 4 * 5 = 600
  ...
  def test_all_iteration_methods_work_on_any_collection_not_just_arrays
    ...
    File.open("example_file.txt") do |file|
      upcase_lines = file.map { |line| line.strip.upcase } # .strip returns a copy of str with leading and trailing whitespace removed.
